...
public class TrainingApp {
            ...
            do{
                ...
                switch(opcionElegida){
                    case QUERY_CITIES:
                        loadAllCities(dam);
                        break;
                    case QUERY_CITIES_BY_CONTENT:
                        searchFilmsByContent(dam);
                        break;
                    case QUERY_FILMS_BY_LENGTH_RANGE:
                        searchFilmsByLengthRange(dam);
                        break;
                    case UPDATE_FILM_DESCRIPTION:
                        updateFilmByTitle(dam);
                        break;
    ...
    //***************************** FUNCIONES LANZADAS DESDE LA ELECCION DEL MENU DE LA APLICACION *****************************
    
    private static void loadAllCities(DataAccessManager dam) throws SQLException {
        List<City> allCities = dam.loadAllCities();
            printCities(allCities);
    }
    
    private static void searchFilmsByContent(DataAccessManager dam) throws SQLException{
        String content = requestCityContentLike();
        List<City> citiesFilteredByInitialLetter = dam.loadCitiesContaining(content);
        printCities(citiesFilteredByInitialLetter);
    }
    
    private static void searchFilmsByLengthRange(DataAccessManager dam) throws SQLException{
        short lengthFrom = requestLengthFrom();
        short lengthTo = requestLengthTo();
        List<Film> filmsFilteredByReleaseYearRange = dam.loadFilmsByLengthRange(lengthFrom, lengthTo);
        printFilms(filmsFilteredByReleaseYearRange);
    }
    
    private static void updateFilmByTitle(DataAccessManager dam) throws SQLException {
                
        String title = requestFilmTitle();
        Film filmByTitle = dam.loadFilmByTitle(title);
        if(filmByTitle!=null){
            System.out.println("\t"+filmByTitle);
            System.out.println("Descripción actual:\n\t"+filmByTitle.getDescription());
            filmByTitle.setDescription(requestNewDescription());
            dam.updateFilm(filmByTitle);
    ...
    //***************************** FIN DE FUNCIONES LANZADAS DESDE LA ELECCION DEL MENU DE LA APLICACION *****************************
    
    
    
    //**************** MÉTODOS DE LECTURA DE DATOS VALIDOS POR TECLADO ********************
    ...
    //**************** FIN MÉTODOS DE LECTURA DE DATOS VALIDOS POR TECLADO ********************
